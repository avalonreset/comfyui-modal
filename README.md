...
- ComfyUI HTTP API exposed via Modal `web_server` on port `8188`
- A separate SaaS endpoint that accepts `{ user_id, ollama_url, workflow_json }`
...
- `...-comfy-api.modal.run` (raw ComfyUI API)
